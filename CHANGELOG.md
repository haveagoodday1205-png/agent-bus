...
- Adds a conformance result JSON Schema plus `agent-bus protocol validate-result` / `npm run protocol:certify:check` so certification JSON, Markdown, and badge artifacts can be verified before adapter projects publish them.
...
- Makes CLI gateway calls fail faster with actionable Central reachability guidance, adds `agent-bus doctor --local-only`, prints human doctor summary counts, and hydrates active room details concurrently for faster `agent-bus status` output.
...
- Highlights the README first-run no-quota proof and clarifies the issue-to-PR demo's current proves / does-not-prove boundaries for new evaluators.
...
- Requeues Python Central long-poll tasks when an edge poll response hits a client disconnect, preventing work from being delivered to a dead request and lost before the edge reconnects.
