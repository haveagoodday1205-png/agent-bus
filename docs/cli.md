...
Telegram buttons are contextual instead of being attached to every reply. `/status` and `/help` show a compact menu, `/new`, `/agents`, and `/agent` show multi-select agent buttons for the active process, `/resume` shows process/thread buttons, and `/rooms` shows room buttons. Room detail replies include a Doctor button that runs `/room doctor <room>` on Python Central and returns a compact diagnosis with stale/failed/contract counts plus recommended next commands. `/room new` starts a room draft with multi-select agent buttons plus step presets; send the room goal as the next plain message, or use `/room start <goal>` to create the room. Telegram callback queries go through the same webhook handler as typed commands; dry-run mode records the `reply_markup` in `notifications.jsonl` so deployments can test the UX without contacting Telegram.
...
For an existing room, `/room doctor room_xxx` is a read-only operator check suitable for mobile triage before waking, pausing, retrying, or creating a follow-up room.
...
agent-bus room event-log room_xxx --json --reports-only --gateway https://YOUR-DOMAIN/agent-bus --token ...
...
Use `agent-bus room health ROOM_ID` for the first operator view of a live or overnight room. It combines the room snapshot and `/nodes` inventory into per-agent status, latest run id, edge session, lease state, attempt number, failure class/category, retryability, recommended action, duplicate active runs, REPORT/DONE contract state, last wake reason, stale state, recovery actions, and safe operator hints. Add `--json` when a monitoring script or Telegram control surface should consume the same object. Runs also carry the same structured attempt ledger in `attempt`/`attempts` and the gateway appends lifecycle snapshots to `run_attempts.jsonl`, so scripts can tell an upstream transient error from auth/config failure without parsing raw stderr.
...
`agent-bus room doctor ROOM_ID` is the compact operator diagnosis for a room. The server combines stale/orphan run inspection, duplicate active run detection, failed-agent retry taxonomy, and REPORT/DONE contract inspection into one object with recommended commands. Failed attempts include `failure_class`, `failure_category`, `retryable`, and `recommended_action`, so Telegram buttons or runbooks can show whether the next move is retry, backoff, fix auth/model config, inspect the agent contract, or inspect the edge runtime. If a completed or paused room is missing an expected `REPORT`, `DONE`, or whole agent run, doctor marks the room as `completed_with_contract_gaps` or `contract_gaps` and recommends creating a follow-up room instead of trying to wake a terminal room.

`agent-bus room follow-up ROOM_ID` turns that terminal-room recommendation into an executable room creation flow. It reads the source room plus doctor output, selects contract-gap agents first, falls back to blocking agents or the original room agents, and wakes the selected agents by default. Use `--dry-run` to preview the request body, `--agents a,b` to override inferred agents, `--no-wake` to create the room without immediate dispatch, and `--goal` or `--reason` to shape the follow-up task.

`agent-bus room retry-failed ROOM_ID` is a dry run by default. Add `--yes` to re-open the room and wake only failed online agents with the latest failure context. Automatic retry is now taxonomy-aware: `upstream_transient`, `rate_limited`, and `timeout` are retry-safe; `auth_config`, `protocol_violation`, `local_runtime`, and unknown failures are blocked until an operator fixes the cause or adds `--force`. It does not retry queued/running agents, agents whose latest run already completed, or agents that are currently offline/unknown. If the original room hit `max_steps`, the retry command expands that room's step budget just enough for the confirmed retry runs.
