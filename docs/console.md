...
- Enter a room through a group-chat layout with a room list, sticky chat composer, room header, and Central's normalized chat-history endpoint (`GET /rooms/{room_id}/chat`). The main chat stream shows agent conversation only, while REPORT/BLACKBOARD/RUNS details stay in the side timeline.
