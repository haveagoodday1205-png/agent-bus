...
| Telegram bot token | Central service environment only | Sends optional operator notifications for central startup, edge registration, run completion, and room completion | Do not put it in room prompts, edge configs, public logs, or GitHub issues |
