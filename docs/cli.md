...
Use `agent-bus room health ROOM_ID` for the first operator view of a live or overnight room. It combines the room snapshot and `/nodes` inventory into per-agent status, latest run id, edge session, lease state, REPORT/DONE contract state, last wake reason, stale state, recovery actions, and safe operator hints. Add `--json` when a monitoring script or Telegram control surface should consume the same object.
